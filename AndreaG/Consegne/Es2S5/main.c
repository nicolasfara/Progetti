...
int main()
{
    ...
    do //Richiesta valori per il primo collega
    {
        ...
        check=scanf("%[\n]",&col[cont]); //Creazione variabile di controllo per verificare che i valori siano corretti
        fflush(stdin);
        ...
        cont++;
    }while(check!=0 && cont<=31);//Altro controllo per verificare che i valori siano conformi
    col[cont-1]=0;
