...
int inHead(TipoLista *head, int elem) {
    List *tmp;
...
/* Inserisce l'elemento in coda,
    ...
    0 se la funzione � terminata con successo */
int inTail(TipoLista *head, int elem) {
    List *tmp, *prec = NULL;
    tmp = (List *)malloc(sizeof(List));
    if(tmp == NULL)
        return 1;
...
int inElem(TipoLista *head, int find, int elem) {
    List *tmp, *prec = NULL;
    tmp = (List *)malloc(sizeof(List));
    if(tmp == NULL)
        return 1;
...
int delElem(TipoLista *head, int elem) {
    //Controllo se la lista � vuota
    if(*head == NULL)
        return 1;
